function rJPEG2TIFF_ranveer(filename,varargin)
...
if isempty(emissivity);emissivity=str2num(exifdata{strcmp(exifdata(:,1), 'Emissivity'), 2});end %get emissivity
if isempty(distance);distance=strsplit(exifdata{strcmp(exifdata(:,1), 'Object Distance'), 2});distance=str2num(distance{1});end %get object distance
if isempty(Tr);Tr=strsplit(exifdata{strcmp(exifdata(:,1), 'Reflected Apparent Temperature'), 2});Tr=str2num(Tr{1});end %get reflected temperature
if isempty(Ta);Ta=strsplit(exifdata{strcmp(exifdata(:,1), 'Atmospheric Temperature'), 2});Ta=str2num(Ta{1});end %get atmospheric temperature
if isempty(RH);RH=strsplit(exifdata{strcmp(exifdata(:,1), 'Relative Humidity'), 2});RH=str2num(RH{1});end %get relative humidity

%extract Planck formula constants (determined when camera was calibrated by FLIR)
B=str2num(exifdata{strcmp(exifdata(:,1), 'Planck B'), 2}); %get Planck B
F=str2num(exifdata{strcmp(exifdata(:,1), 'Planck F'), 2}); %get Planck F
O=str2num(exifdata{strcmp(exifdata(:,1), 'Planck O'), 2}); %get Planck O
R1=str2num(exifdata{strcmp(exifdata(:,1), 'Planck R1'), 2}); %get Planck R1
R2=str2num(exifdata{strcmp(exifdata(:,1), 'Planck R2'), 2}); %get Planck R2

%extract atmospheric (determined when camera was calibrated by FLIR)
A1=str2num(exifdata{strcmp(exifdata(:,1), 'Atmospheric Trans Alpha 1'), 2}); %get Atmospheric Transmissivity Alpha 1
A2=str2num(exifdata{strcmp(exifdata(:,1), 'Atmospheric Trans Alpha 2'), 2}); %get Atmospheric Transmissivity Alpha 2
B1=str2num(exifdata{strcmp(exifdata(:,1), 'Atmospheric Trans Beta 1'), 2}); %get Atmospheric Transmissivity Beta 1
B2=str2num(exifdata{strcmp(exifdata(:,1), 'Atmospheric Trans Beta 2'), 2}); %get Atmospheric Transmissivity Beta 2
X=str2num(exifdata{strcmp(exifdata(:,1), 'Atmospheric Trans X'), 2}); %%get Atmospheric Transmissivity X
